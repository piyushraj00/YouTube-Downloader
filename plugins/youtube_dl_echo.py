...
async def echo(bot, update):
    ...
    if Config.UPDATE_CHANNEL:
        try:
            user = await bot.get_chat_member("@YouTubeVideoDownloaderService", update.from_user.id)
            if user.status == "kicked":
              await bot.edit_message_text(text=Translation.BANNED_USER_TEXT, message_id=fmsg.message_id)
              return
        except UserNotParticipant:
            await bot.edit_message_text(chat_id=update.chat.id, text=Translation.FORCE_SUBSCRIBE_TEXT, message_id=fmsg.message_id, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="😎 Join Official Channel 😎", url=f"https://telegram.me/YouTubeVideoDownloaderService")]]))
            return
        except Exception:
